...
function handleEmojiClick(event, memeId, reactionType) {
    ...
    fetch('https://script.google.com/macros/s/AKfycbyVharq_djN8-LwxH5ErucNZ5hiqgUPyGdXRuhMTx7qDOClTGu3KTYvIQqJLlSsyH-YPw/exec', {
        method: 'POST',
        body: JSON.stringify({
            memeId: memeId,
...
function fetchMemes() {
    ...
    .then(data => {
        ...
        shuffledData.forEach((row) => {
            ...
            <p class="username-class" style="margin-top: -10px; margin-left: 0px;"><strong>${row.username}:</strong> ${row.text}</p>
